  ...
  <ul class="ul-item-list">
    ...
    >
      <img
        ...
        class="img-item"
        :class="{
          'item-selected': parseInt(item.id) == $store.state.headgear_bottom_item.id,
...
export default {
  ...
  data() {
    return {
      items: this.$store.state.itens.bot,
      active: false,
    };
  },
  methods: {
    ...
    validBot: function () {
      ...
      // TOP
      for (let i = 0; i < item_fortmat.location.length; i++) {
        if (state_fortmat.headgear_top_item.location === undefined)
          break;
        if (
      ...
      this.SAVE_HEADGEAR_BOTTOM_ITEM(item_fortmat);
    },
